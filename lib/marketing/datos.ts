...
export async function cargarMarketing(
   ...
   */
  const [pauta, personas, estado, rendimiento, rendimientoAntes, creatividades, borradores] =
    await Promise.all([
  ...
  /* ── Leads ──────────────────────────────────────────────────────────────── */
  const leads: Lead[] = personas.map((p) => aLead(p, metaPorAnuncio));
  ...
  for (const a of anuncios) {
    ...
    c.avanzados += a.cotizaciones + a.agendadas;
    c.ventas += a.ventas;
    c.cobrado += a.cobrado;
    c.anuncios += 1;
  ...
  const campanas = [...porCampana.values()].map((c) => ({
    ...
    roas: c.gasto ? c.cobrado / c.gasto : null,
  }));
  ...
  const embudo = armarEmbudo({
    ...
    calificados,
    avanzados: pauta.propios.cotizaciones + pauta.propios.agendadas,
...
function aLead(
  ...
  return {
    ...
    llegoEn: p.desde,
    etapa: p.compro && etapa !== "perdido" ? "ganado" : etapa,
    calificado,
    cotizo: p.cotizo,
    agendo: p.agendo,
    compro: p.compro,
