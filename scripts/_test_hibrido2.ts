...
async function main() {
  ...
  sent = [];
  let r = await manejarEntranteWaha(ev(TXT, true, "ECOX"), { enviar: mockEnviar });
  ...
  sent = [];
  r = await manejarEntranteWaha(ev("Hola Juan, yo te sigo atendiendo", true, "HUMX"), { enviar: mockEnviar });
